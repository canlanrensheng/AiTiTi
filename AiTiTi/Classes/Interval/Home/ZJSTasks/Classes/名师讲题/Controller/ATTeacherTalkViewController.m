...
@interface ATTeacherTalkViewController ()<UICollectionViewDataSource,UICollectionViewDelegate>
...
@implementation ATTeacherTalkViewController
...
- (void)setUpNaviAttributes{
    ...
    self.controllersClass = @[@"ATExamClasssifyViewController",@"ATExamPaperViewController"];
...
- (NSMutableArray *)controllers{
    if (!_controllers) {
        ...
        for(int i = 0; i < self.controllersClass.count;i++){
            ATExamClasssifyViewController *listVC = [[ATExamClasssifyViewController alloc] init];
            [self addChildViewController:listVC];
            [controllers addObject:listVC];
            
            ATExamPaperViewController *mapVC = [[ATExamPaperViewController alloc] init];
...
- (UISegmentedControl *)segmentControl{
    if(!_segmentControl){
        NSArray *segmentedArray = [NSArray arrayWithObjects:@"考点分类",@"试卷",nil];
...
//        _segmentControl.tintColor = DEFAULTCOLOR;
        ...
        [_segmentControl setBackgroundImage:[UIImage ZJS_imageWithColor:DEFAULTCOLOR]
