...
#import "ATSystemClassModel.h"
#import "ATSystemClassViewModel.h"
#import "ATSystemClassDetailViewController.h"

#define kSystemClassHeaderHeight 50

@interface ATSystemClassViewController ()
@property (nonatomic, strong) JPullDownMenu *menu;
@property (nonatomic, strong) ATSystemClassViewModel *viewModel;
...
- (void)tx_configSubViews {
    ...
    [self.tableView mas_updateConstraints:^(MASConstraintMaker *make) {
        make.edges.equalTo(self.view).insets(UIEdgeInsetsMake(kSystemClassHeaderHeight, 0, 0, 0));
    }];
    
}

- (void)setUpMenuView {
    self.menu = [[JPullDownMenu alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, kSystemClassHeaderHeight) menuTitleArray:@[@"工作岗位",@"薪资",@"工作经验"]];
    NSArray *regionArray = @[
                             @{@"title":@"工作岗位",@"image":@""},
                             @{@"title":@"iOS开发",@"image":@""},
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    ATSystemClassDetailViewController *detailVC = [[ATSystemClassDetailViewController alloc] init];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    ATSystemClassModel *model = self.viewModel.dataArray[section];
    @weakify(self);
...
- (ATSystemClassViewModel *)viewModel {
    if (!_viewModel) {
        _viewModel = [[ATSystemClassViewModel alloc] init];
    }
    return _viewModel;
}
